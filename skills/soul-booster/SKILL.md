...
推理分层系统（借鉴 Claude Code）：轻推理/深度推理/极致推理三层，详见 [references/thinking_tier_system.md](references/thinking_tier_system.md)。
自动上下文压缩（借鉴 Claude Code）：Token 阈值触发 + 熔断机制，详见 [references/auto_compact_system.md](references/auto_compact_system.md)。
工具执行摘要（借鉴 Claude Code）：Haiku生成30字符摘要，git commit风格，详见 [references/tool_use_summary.md](references/tool_use_summary.md)。
工具并发控制（借鉴 Claude Code）：读并发/写串行 + 上下文修改排队，详见 [references/tool_concurrency.md](references/tool_concurrency.md)。
Fork Subagent 模式（借鉴 Claude Code）：隐式分叉，共享上下文+cache，详见 [references/fork_subagent_pattern.md](references/fork_subagent_pattern.md)。
Forked Agent 模式（借鉴 Claude Code）：影子 agent 后台执行，共享上下文，详见 [references/forked_agent_pattern.md](references/forked_agent_pattern.md)。
...
四个上位协议文件见 [references/Meta_Logic_Matrix.md](references/Meta_Logic_Matrix.md)、[references/Cognitive_Bias_Firewall.md](references/Cognitive_Bias_Firewall.md)、[references/Understanding_Protocol.md](references/Understanding_Protocol.md)、[references/Predictive_Engine.md](references/Predictive_Engine.md)。
三库内化方案见 [references/three_library_internalization_plan.md](references/three_library_internalization_plan.md)。
